...
STT_1,
/ goto M_PRIME : calculate the max prime
...
SHF_T,	
		...
		INC
		ADD R_0			/ A <- X - 10
		SPA				/ X >= 10 then skip next step
		BUN P_NO_T
		BUN NXT_T

NXT_T,					/ X >= Q then Q <- Q << 1
		CLE
		ISZ CNT_T
		LDA TEN
		CIL
...
CHK_N,	HEX 0
		...
		ADD VM3
		SPA
		BUN CHK_N I		/ LAST < 3 then finish this "N"
		LDA LAST
		ADD VM1
		STA Y		
		BUN STT_1
...
_X,	DEC 1		/ M[_X]
_Y,	DEC 1		/ M[_Y]
_R,	DEC 0		/ M[_R] - (_Y, _R) return (1, 0) then _N is prime
_P,	DEC 0 		/ M[_P]
